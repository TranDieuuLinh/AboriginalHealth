    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        error => {
          ...
          // Default location coordinates for Sydney
          const defaultSydneyCoordinates = [151.2093, -33.8688]; 
          resolve(defaultSydneyCoordinates); // Resolve with Sydney coordinates
        }
      );
    } else {
...
const getDirections = async (start, end) => {
  ...
  try {
    ...
    return { routeCoordinates: [], distance: 0 };  
  }
};
